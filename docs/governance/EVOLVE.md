...
生产工作从 SKILL.md 的 Execution Router 进入；修改 Skill 本身时使用本文。无需复制新的业务目录，也不新建另一套工作流引擎。
...
.\scripts\govern.ps1 impact --from runtime.blender_mcp --from runtime.ssh_transport
```
...
.\scripts\govern.ps1 test --affected-by runtime.asset_router
.\scripts\govern.ps1 contract-test
.\scripts\govern.ps1 test --requirement REQ-NEW-ID
.\scripts\govern.ps1 accept --requirement REQ-NEW-ID --predicted-impact runtime.asset_router --check
```

8. 审查警告及代表性的真实操作。全部通过后去掉 --check 接受版本；changes 由 skillctl 生成，不手写成功证据。失败时修实现，不能仅为了通过而删除历史断言。

```powershell
.\scripts\govern.ps1 accept --requirement REQ-NEW-ID --predicted-impact runtime.asset_router
```
...
govern.ps1 调用当前项目的 govern.py，后者使用外部 Contract-Governed Skill 工具。默认工具位置为 D:/contract-govern skill；可用环境变量 CONTRACT_GOVERN_HOME 或 --framework 指定其他位置，--python 可以指定已安装 skillctl 及测试依赖的解释器。它不会修改参考工具项目。

治理在源码副本中运行，复制范围由 scripts/govern.py 的 DIRECTORIES/FILES 明确列出。实际 projects、.deps、.skillctl 缓存不进入测试副本和版本快照。新顶层源码目录必须加入该列表；新增普通 runtime、docs、tests 等目录内文件会自动纳入。独占锁及前后摘要用于检测并发修改。完整结果保存在 .skillctl/reports/，成功记录回写 changes/。
...
当前历史回归登记为一个 95 项测试集合；影响分析保守地保留整个集合。后续可新增更细的专项测试用于快速定位，不能把现有历史集合删掉以缩小验收范围。
