...
- runtime/cli.py
Action:
- For schema 0.2, use `final-review-context` to inspect the current immutable completed preview, critic and provenance snapshot, then record only the explicit user decision with `final-review-v02`. On approval use `deliver-v02` with the current expected version to publish the verified local package. Keep package paths project-relative on macOS and Windows; the runtime handles long physical Windows paths without changing the inventory. On a final rejection, return to Step 7V for one new concrete revision-required critic; then continue through Step 8V for the existing bounded revision flow before a new preview and final review. No remote generation or artistic approval is implied by successful checks.
...
- For user input, run style-inspect on the directory containing profile.yaml. Keep format_version independent of the user's release version. Report invalid definitions or pending review/resources; never fill in approval just to pass a validator. Use style-prepare with a new output directory when calibration is authorized, execute its guarded request through MCP and inspect actual output. After real design review, retain the named reviewer and scoped evidence; this does not replace user artistic approval.
- Import a complete package with style-import PROJECT PACKAGE --expected-version N through ManifestManager before visual approval. Keep the source and previous imported versions. Use style-resolve --project PROJECT for project inputs. Return to Step 0V to select the exact imported ID/version for a scene, or END after standalone work. Adding a compatible user package needs no Skill code edit.
Next:
- Step 0V
- END
