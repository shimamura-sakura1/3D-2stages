...
主控 Agent 拆解任务、制定相机和灯光方向、查看实际渲染，并根据评审安排受限修订。
结构化视觉任务把判断与执行连接起来；Python 校验输入、编译计划、保存每轮来源与进度。
中断后先检查已有任务和渲染凭据，再继续下一步。详见 [任务追踪](docs/v02/task-tracking.md)。
...
为视觉判断提供有来源的倾向。风格包的检查、校准与导入见 [用户风格](docs/v02/user-styles.md)。
...
- 真实 GPU 生成、画面质量和视觉判断需要针对当前输入验证，不能由接口测试代替。
