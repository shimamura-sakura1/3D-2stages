...
def validate_scene_plans(manager,m,plans):
    ...
    for item in mappings:
        ...
        resolve_material(docs['style_assignment']['style_profile'],item['material_class'],item['condition'],version=docs['style_assignment']['profile_version'],project_root=manager.root)
...
def prepare_scene(manager):
    ...
    materials={x['object_id']:resolve_material(style['id'],x['material_class'],x['condition'],version=style['version'],project_root=manager.root) for x in plans['semantic_material_map']['mappings']}
    ...
    dependencies.extend(inside(style_root,path) for path in style_files(style) if inside(style_root,path).is_file())
